...
from src.modules.detect_ions  import *
from src.modules.detect_gaps  import *
from src.modules.superlearner import *
from src.modules.rosetta_descriptors import Get_descriptors
...
def post_processing(pdbfiles, partner1, partner2, trainedmodels, mlmodel, st):
    for mol, pdb in enumerate(pdbfiles):
       ...
       # 2. checks if the original structure contains ion(s)
        ...
        # ------------------------------------------------------------------------
        train_file_columns = pd.read_csv(f"{PbeePATH}/in/PPBE_model/{version}__pbee_train_file.csv")
        ...
        x_train = pd.read_csv(f'{PbeePATH}/in/PPBE_model/{version}__pbee_train_file.csv', delimiter=',').drop(columns=columns_to_remove)
        y_train = pd.read_csv(f'{PbeePATH}/in/PPBE_model/{version}__pbee_train_file.csv', delimiter=',')['dG_exp']
...
def pdbcleaner(pdbfile, basename, outdir, submit_dir, partner1, partner2):
    print("CHECKING SUBDIR =======", submit_dir)
    commands = [
        f'python {PbeePATH}/src/modules/clean_pdb.py {pdbfile} {partner1}',
        f'python {PbeePATH}/src/modules/clean_pdb.py {pdbfile} {partner2}',
    ...
    for command in commands:
        subprocess.run(command, stdout=subprocess.PIPE, shell=True, cwd=submit_dir)
...
def configure_mlmodels(PbeePATH):
    trainedmodels = [
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_LinearRegression.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_ElasticNet.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_SVR.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_DecisionTreeRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_KNeighborsRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_AdaBoostRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_BaggingRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_RandomForestRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_ExtraTreesRegressor.pkl',
        f'{PbeePATH}/in/PPBE_model/{version}__basemodel_XGBRegressor.pkl']
...
if (__name__ == "__main__"):
    ...
    PbeePATH = PROJECT_ROOT

    # Define ML model
    trainedmodels = configure_mlmodels(PbeePATH)
    mlmodels = {
        'sl': f'{PbeePATH}/in/PPBE_model/{version}__SuperLearner.pkl',
        'lr': trainedmodels[0],
        'en': trainedmodels[1],
        'sv': trainedmodels[2],
        'dt': trainedmodels[3],
