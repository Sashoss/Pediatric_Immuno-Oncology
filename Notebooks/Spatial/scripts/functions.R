...
preProcessSeuratObjectSpatial<- function(seurat_object, normalization = "SCT"){
    if(normalization == "SCT"){
      #Load10X_Spatial(file) %>% 
        seurat_object %>%
        PercentageFeatureSet(pattern="^MT-", col.name="percent.mt") %>%
...
anchorMapping1 <- function(reference, query, feats, query.dims=15, normalization.method = "LogNormalize", anchor.labels,save.loc=FALSE){
...
anchorMapping <- function(reference, query, feats, query.dims=15, normalization.method = "LogNormalize", anchor.labels, save.loc=FALSE){
  ...
  # Identify non-mapped features
  non_mapping_indices <- which(rowSums(predictions.assay@data) == 0)
  non.mapping <- rownames(predictions.assay@data)[non_mapping_indices]

  # Optionally, print non-mapped features
  # print(paste("Non-integrated features:", paste(non.mapping, collapse = ", ")))
