...
def calculate_position_wise_quadratic_mean(runs_folder, runs=['Run_MD_chain1', 'Run_MD_chain2', 'Run_MD_chain3']):
    """
    ...
    atom_numbers = None
    for run in runs:
        if "normal" in runs_folder:
            rmsf_file = os.path.join(runs_folder, run, 'rmsf_c'+run[-1]+'.xvg')
        elif "phospho" in runs_folder:
            rmsf_file = os.path.join(runs_folder, run, 'rmsf_p'+run[-1]+'.xvg')
...
def main():
    base_dir = "." 
    folders = ['../../out/Minima1/Minima1_normal_SPP1', '../../out/Minima1/Minima1_phospho_SPP1']
    results = {}

    for folder in folders:
        runs_folder = os.path.join(base_dir, folder, 'md')
        atom_numbers, quadratic_mean = calculate_position_wise_quadratic_mean(runs_folder)
        if quadratic_mean is not None:
            results[folder] = (atom_numbers, quadratic_mean)
            output_file = os.path.join(base_dir, f"{folder}_quadratic_mean.xvg")
